...
plotly::ggplotly(ggplot(escape_harvest, aes(fill=type, y=count/1000000, x=year)) + 
...
kobe_df <- data.frame(S = spwn_df$mid, 
                      U = er_df$mid, 
                      year = spwn_df$year) |>
  mutate(U_Umsy = U/benchmarks[3,1], 
         S_Smsy = S/(benchmarks[1,1]*1.25)) #correct from 80%Smsy to Smsy

plotly::ggplotly(ggplot(kobe_df, aes(S_Smsy, U_Umsy)) +
