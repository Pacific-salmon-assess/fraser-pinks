...
```{r escape-harvest plot, echo=FALSE, fig.align = "center", fig.height = 4, fig.width = 5}
...
```{r parameter-hist-plot, message = FALSE, warnings = FALSE, fig.align = "center", fig.height = 4, fig.width = 5}
post_df <- data.frame(
...
```{r sr-plot, warnings = FALSE, fig.align = "center", fig.height = 4}
ggplot() +
...
And recruitment deviations over time which suggest little evidence for directional change in productivity, though last three brood years have all been below average. 
```{r resid-plot, warnings = FALSE, fig.align = "center", fig.height = 4}
...
Here are estimated benchmarks, along with 25th and 50th percentiles which have been proposed as alternatives to spawner-recruitment benchmarks for data limited stocks ([Holt et al. 2018](https://www.dfo-mpo.gc.ca/csas-sccs/Publications/ResDocs-DocRech/2018/2018_011-eng.html)). Under the Wild Salmon Policy 80% of Smsy is typically used as the upper biological benchmark, but here I am just presenting Smsy. 
```{r }
# Sgen function
...
rownames(benchmarks) <- c("Smsy","Sgen","Umsy","25th percentile","50th percentile")
...
```{r exploit-rate-plot, warnings = FALSE, fig.align = "center", fig.height = 4}
er <- model_pars$U
...
And here is what reconstructed spawner abundance looks like over time relative to Smsy and Sgen. 
```{r status-plot, warnings = FALSE, fig.align = "center", fig.height = 4}
spwn <- model_pars$S
...
ggplot(spwn_df, aes(x=year, y = mid), show.legend = F) +
  ...
  annotate("text", label = "Smsy", x = 1961, y = 5.95) +
