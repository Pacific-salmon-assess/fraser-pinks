...
## Introduction and Context *(biology, environmental conditions & fishery)*

Fraser Pink Salmon return to the Fraser River in odd-numbered years as two year old fish. They are currently classified as a single conservation unit (CU) for Wild Salmon Policy and one stock management unit for implementation of Fish Stock Provisions.

The same abundance-based harvest control rule has been in place for Fraser Pinks since the mid-1980s.Fisheries on Fraser Pinks are managed in-season by the Fraser Panel process of the Pacific Salmon Treaty.
...
plotly::ggplotly(p) |>
  ...
  layout(hovermode = "x unified") #displays all x info (i.e. stacked)
```
...
```{r parameter-hist-plot, warning = FALSE, out.width = "75%"}
post_df <- data.frame(
...
```{r SR-plot, warning = FALSE, out.height='80%', out.width = '80%'}
p <- ggplot() +
...
```{r resid-plot, warning = FALSE, out.width = "75%"}
...
Here are estimated benchmarks (all values are in units of millions of fish except $U_{MSY}$). Under the Wild Salmon Policy 80% of $S_{MSY}$ is typically used as the upper biological benchmark, and $S_{GEN}$ as the lower biological benchmark ([Holt and Bradford. 2011](https://www.tandfonline.com/doi/abs/10.1080/02755947.2011.578525)). Also shown are the 25^th^ and 50^th^ percentiles which have been proposed as alternative lower and upper benchmarks for data limited stocks ([Holt et al. 2018](https://www.dfo-mpo.gc.ca/csas-sccs/Publications/ResDocs-DocRech/2018/2018_011-eng.html)). 
```{r benchmarks}
# Sgen function
...
This is what reconstructed harvest rate looks like over time relative to $U_{MSY}$.  
```{r exploit-rate-plot, warning = FALSE, out.width = "75%"}
er <- model_pars$U
...
And here is what reconstructed spawner abundance looks like over time relative to upper (80% $S_{MSY}$) and lower ($S_{GEN}$) biological benchmarks. 
```{r status-plot, warning = FALSE, out.width = "75%"}
spwn <- model_pars$S
...
```{r profile-plot, warning = FALSE, out.width = "85%"}
spw <- seq(0,max(brood_t[,4]),length.out=100)
...
```{r kobe-plot, warning = FALSE, out.height='80%', out.width = '80%'}
