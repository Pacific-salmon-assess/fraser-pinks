...
```{r coarse kobe, out.height='80%', out.width = '80%', fig.cap= "Kobe plot of estimated spawner abundance and exploitation rate relative to S~MSY~ (x-axis) and U~MSY~ (y-axis), respectively, over time. Highlighting past data (light grey to darker grey) and current state (black) with 95% CIs."}
...
```
```{r alt-hcr, out.height='80%', out.width = '80%', fig.cap= "OPTIONAL NEW Inferred Fraser pink salmon harvest control rule with estimated Limited Reference Point (S~GEN~; LRP), 80%S~MSY~, and U~MSY~ (medians +/- 95% credible intervals)."}
#chuck suggested plotting the data over the HCR
hcr_data <- left_join(select(data, year, harvest, spawn),
                      select(kobe_df, year, U), 
                      by = "year") |>
  mutate(run_size_mils = (harvest+spawn)/1000000)


p <- ggplot(hcr, aes(x=run_size, y=er)) + 
  theme_sleek() +
  xlab("Run size (millions)") +
  ylab("Target exploitaiotn rate") +
  geom_segment(aes(y = Umsy, yend = Umsy, x = 0, xend = 30), color = "blue", lwd = 0.5, alpha=0.01,linetype="dotted") +
  ...
  annotate("text", x = run_usr +1, y = 1.02, label = expression(paste("80%",S)[MSY]), color = "black", size=3) +
  annotate("text", y = Umsy +0.03, x = 30, label = expression(paste(U)[MSY]), color = "blue", size=3) +
  geom_point(data = hcr_data, aes(run_size_mils, U, text = year, color = year)) +
  scale_color_viridis_c()

#plotly::ggplotly(p, tooltip = "text") |>  #text year is getting confused with the other text layers
#  config(displayModeBar = FALSE) 
p
```
 \
