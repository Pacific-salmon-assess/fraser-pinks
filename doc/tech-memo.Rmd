...
kable(data.frame(benchmarks.df), align="l") |> 
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
```{r, Sgen 80 perc Msy, out.width = "75%"}
...
ggplot(er_df, aes(x=year, y = mid), show.legend = F) +
  ...
  theme_bw()
```
```{r, yield and recruit profiles, out.width = "85%"}
...
```{r SR plot, out.height='80%', out.width='90%'}
p <- ggplot() +
...
```{r recruitment residuals, out.width = "75%"}
