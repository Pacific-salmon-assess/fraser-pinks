...
p <- ggplot(kobe_df, aes(S_Smsy, U_Umsy)) +
  geom_errorbar(aes(x = S_Smsy , y = U_Umsy, ymin = U_Umsy_LCI, ymax = U_Umsy_UCI),
                colour="grey", width=0, size=0.3) +
  geom_errorbarh(aes(x = S_Smsy , y = U_Umsy, xmin = S_Smsy_LCI, xmax = S_Smsy_UCI),
...
```{r trackdown controls, include = FALSE}
#trying to write .Rmd to google docs then use trackdown to get updates. 


#library(trackdown) #install from CRAN if you don't have it already

#trackdown::upload_file(file = here("doc/tech-memo.Rmd"), 
#                       hide_code = TRUE)
```